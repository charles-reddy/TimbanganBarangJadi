...
class Laptiketmuatapproved extends Component
{
    ...
    public function render()
    {
        if ($this->katakunci  !=null) {
            $data = DB::connection('sqlsrv')->table('create_t_m_s')->join('createsppbs', 'createsppbs.id', 'create_t_m_s.tmSppbID')->join('customers', 'customers.custID', 'create_t_m_s.custID')->select('create_t_m_s.id','pendfNo', 'tmQtyKg','tmQtyKarung','sppbNo','tmCarID','isMktApp','tglMuat','custName','isSecCek','tmTranspName')->where('isMktApp','1')->where('pendfNo','like','%' . $this->katakunci . '%')->orderBy('id','desc')->paginate(10);
        } elseif ($this->katacust  !=null) {
            $data = DB::connection('sqlsrv')->table('create_t_m_s')->join('createsppbs', 'createsppbs.id', 'create_t_m_s.tmSppbID')->join('customers', 'customers.custID', 'create_t_m_s.custID')->select('create_t_m_s.id','pendfNo', 'tmQtyKg','tmQtyKarung','sppbNo','tmCarID','isMktApp','tglMuat','custName','isSecCek','tmTranspName')->where('isMktApp','1')->where('custName','like','%' . $this->katacust . '%')->orderBy('id','desc')->paginate(10);
        } elseif ($this->tglMuat  !=null) {
            $data = DB::connection('sqlsrv')->table('create_t_m_s')->join('createsppbs', 'createsppbs.id', 'create_t_m_s.tmSppbID')->join('customers', 'customers.custID', 'create_t_m_s.custID')->select('create_t_m_s.id','pendfNo', 'tmQtyKg','tmQtyKarung','sppbNo','tmCarID','isMktApp','tglMuat','custName','isSecCek','tmTranspName')->where('isMktApp','1')->wheredate('tglMuat','=', $this->tglMuat)->orderBy('id','desc')->paginate(10);
        } else {
            $data = DB::connection('sqlsrv')->table('create_t_m_s')->join('createsppbs', 'createsppbs.id', 'create_t_m_s.tmSppbID')->join('customers', 'customers.custID', 'create_t_m_s.custID')->select('create_t_m_s.id','pendfNo', 'tmQtyKg','tmQtyKarung','sppbNo','tmCarID','isMktApp','tglMuat','custName','isSecCek','tmTranspName')->where('isMktApp','1')->orderBy('id','desc')->paginate(10);
        // dd($data);
        }
