...
class TimbanganKeluar extends Component
{
    ...
    public function edit($id)
    {
        ...
        // dd($data->spmID);
        $dataspm = DB::connection('sqlsrv')->table('createsppbs')->join('create_t_m_s', 'create_t_m_s.tmSppbID', 'createsppbs.id')->join('createspms', 'createspms.tiketID', 'create_t_m_s.id')->where('createspms.id', $data->spmID)->select('createsppbs.id', 'createsppbs.openQtyKg', 'create_t_m_s.tmQtyKg')->first();
        // dd($dataspm);
    ...
    public function update()
    {
        ...
        switch ($this->itemType) {
            ...
            default:
                try {
                    // Fetch product gross_min and gross_max from database
                    $product = DB::connection('sqlsrv')
                        ->table('products')
                        ->where('itemCode', $this->itemCode)
                        ->select('gross_min', 'gross_max')
                        ->first();

                    if (!$product) {
                        session()->flash('error', 'Product tidak ditemukan');
                        return;
                    }

                    $grossMin = (float) $product->gross_min;
                    $grossMax = (float) $product->gross_max;

                    // Validate avgKarung against dynamic range from products table
                    $isInRange = ($this->avgKarung >= $grossMin && $this->avgKarung < $grossMax);
                    $isApproved = ($this->isApp == 1);
                    ...
                    if ($isInRange || $isApproved) {
                        ...
                        // Out of range and not approved - log and show error
                        DB::connection('sqlsrv')->table('trscale')->where('id', $this->id_trscale)->update([
                            'avgKarung' => $this->avgKarung,
                        ]);
                        ...
                        session()->flash('error', "Avg Karung tidak sesuai range (Min: {$grossMin}, Max: {$grossMax})");
                        return;
                    }

                    // Keep the old switch for backward compatibility (can be removed after testing)
                    /*
                    switch ($this->itemCode) {
                        ...
                        ########   kemasan 1 kg plastik awal - larist ####
                        case 'S2IBE1AB0D':
                            // dd('1 kg plastik');
                            if ($this->avgKarung >= 20.239 and $this->avgKarung < 20.522) {
                                //    dd('1');         
                            ...
                            break;
                        ########   kemasan 1 kg plastik akhir - larist ####
                        ...
                        ########   kemasan 1 kg karton awal- Larist #### 
                        case 'S2IBEVAB0D':
                            // dd('1 kg karton');
                            if ($this->avgKarung >= 20.621 and $this->avgKarung < 20.773) {
                                //    dd('1');         
                            ...
                            break;
                        #######   kemasan 1 kg karton akhir- Larist #### 
                        ...
                        ########   kemasan 500 gram plastik awal-Larist #### 
                        case 'S2IBF1AB0D':
                            // dd('500 gram plastik');
                            if ($this->avgKarung >= 10.159 and $this->avgKarung < 10.271) {
                                //    dd('1');         
                            ...
                            break;
                        #######   kemasan 500 gram palstik akhir-Larist ####
                        ...
                        ########   kemasan 500 gram karton awal-Larist #### 
                        case 'S2IBFVAB0D':
                            //  dd('500 gram karton');
                            if ($this->avgKarung >= 10.414 and $this->avgKarung < 10.531) {
                                //    dd('1');         
                            ...
                            break;
                        #######   kemasan 500 gram karton akhir-larist #### 
                    ...
                    */ // End of commented old switch logic
