...
class Fgdashboard extends Component
{
    ...
    public function render()
    {
        ...
        // dd($data7hari);
        $dataout = DB::connection('sqlsrv')->table('trscale')->join('customers','customers.custID','trscale.custID')->join('products','products.itemCode','trscale.itemCode')->join('createspms','createspms.id','trscale.spmID')->join('create_t_m_s','create_t_m_s.id','createspms.tiketID')->whereNotNull('netto')->where('create_t_m_s.tmQtyKg','>',0)->whereDate('jam_out',Carbon::now())->orderBy('jam_out','desc')->paginate(10);
        // dd($dataout);
