            ...
            <div class="row">
                <div class="col">
                    ...
                    <div class="mb-3 mt-3 row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control w-50" wire:model="suppName" >
                        </div>
                    </div>
                    <div class="mb-3 mt-3 row">
                        <label for="nama" class="col-sm-2 col-form-label">Alamat</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control w-50" wire:model="suppAdd" >
                        </div>
                    </div>
        ...
        <table class="table table-striped table-sortable">
            <thead>
                <tr>
                    ...
                    <th class="col-md-4 sort @if($sortColumn=='suppName') {{ $sortDirection }}   @endif" wire:click="sort('suppName')" >Nama Supplier</th>
                    <th class="col-md-4 sort desc @if($sortColumn=='suppAdd') {{ $sortDirection }}   @endif" wire:click="sort('suppAdd')" >Alamat</th>
                    
                </tr>
            </thead>
            <tbody>
                @foreach ($msupplier as $key => $value)
                <tr>
                    <td></td>
                    <td>{{ $msupplier->firstItem() + $key }}</td>
                    <td>{{ $value->suppName }}</td>
                    <td>{{ $value->suppAdd }}</td>
                    
                    
                    <td>
                        <a wire:click="edit({{ $value->suppID }})" class="btn btn-primary btn-sm">Edit</a>
                    </td>
                </tr>
                @endforeach
