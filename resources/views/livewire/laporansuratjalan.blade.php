        ...
        <table class="table table-striped table-sortable">
            <thead>
                <tr>
                    ...
                    <th class="col-md-2 sort desc @if ($sortColumn == 'custName') {{ $sortDirection }} @endif"
                        wire:click="sort('custName')">Customer</th>
                    <th class="col-md-1 sort desc @if ($sortColumn == 'qtyKg') {{ $sortDirection }} @endif"
                        wire:click="sort('itemName')">Barang </th>
                    <th class="col-md-1 sort desc @if ($sortColumn == 'qtyKg') {{ $sortDirection }} @endif"
                        wire:click="sort('qtyKg')">Bobot SPM </th>
                    <th class="col-md-2 sort desc @if ($sortColumn == 'spmNo') {{ $sortDirection }} @endif"
                        wire:click="sort('spmNo')">SJ No </th>
                    <th class="col-md-2 sort desc @if ($sortColumn == 'sppbNo') {{ $sortDirection }} @endif"
                        wire:click="sort('sppbNo')">SPPB No </th>
                    <th class="col-md-1 sort desc @if ($sortColumn == 'tglSpm') {{ $sortDirection }} @endif"
                ...
                @foreach ($datascaleout as $key => $value)
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                <a href="/cetaksj/{{ $value->id }} " class="btn btn-primary" target="_blank">Cetak
                                    Surat Jalan</a>
                            @endif
                        </td>
                    </tr>
