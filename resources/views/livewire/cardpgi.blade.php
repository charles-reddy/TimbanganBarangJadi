            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="card border-primary mb-3" style="max-width: 150rem;">
                        <div class="card-header  text-center">PGI - {{ $spmNo }}</div>
                        <div class="card-body text-primary">
                            <img class="rounded float-start" style="width: 750px" src="{{ $buktiPGI }}">
                        </div>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="mb-3 mt-3 row">
                        <label for="nama" class="col-sm-2 col-form-label">Daftar Rata-Rata Karung </label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control w-50" wire:model="listkarung" disabled>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-striped table-sortable">
                <thead>
                    <tr>
                        ...
                        <th class="col-md">Shift WB IN</th>
                ...
                <tbody>
                    ...
                    @foreach ($datapgi as $key => $value)
                        <tr>
                            <td></td>
                            <td>{{ $datapgi->firstItem() + $key }}</td>
                            <td>{{ $value->spmNo }}</td>
                            <td>{{ $value->trsID }}</td>
                            <td>{{ $value->sppbNo }}</td>
                            <td>{{ $value->pendfNo }}</td>
                            <td>
                                @if ($value->shift_tm == 'Shift 1')
                                    <span class="badge bg-primary">{{ $value->shift_tm }}</span>
                                @elseif($value->shift_tm == 'Shift 2')
                                    <span class="badge bg-success">{{ $value->shift_tm }}</span>
                                @elseif($value->shift_tm == 'Shift 3')
                                    <span class="badge bg-warning text-dark">{{ $value->shift_tm }}</span>
                                @else
                                    <span class="badge bg-danger">{{ $value->shift_tm }}</span>
                                @endif
                            </td>
                            <td>
                                @if ($value->shift_wbin == 'Shift 1')
                                    <span class="badge bg-primary">{{ $value->shift_wbin }}</span>
                                @elseif($value->shift_wbin == 'Shift 2')
                                    <span class="badge bg-success">{{ $value->shift_wbin }}</span>
                                @elseif($value->shift_wbin == 'Shift 3')
                                    <span class="badge bg-warning text-dark">{{ $value->shift_wbin }}</span>
                                @else
                                    <span class="badge bg-danger">{{ $value->shift_wbin }}</span>
                                @endif
                            </td>
                            <td>{{ $value->driver }}</td>
                            <td>{{ $value->carID }}</td>
                            ...
                            <td>{{ $value->dnNo }}</td>
                            <td>
                                <a wire:click="edit1('{{ $value->trsID }}', '{{ $value->trans_type }}')"
                                    class="btn btn-warning btn-sm">AVG
                                    Karung</a>
                            </td>
                            ...
                            <td>
                                <a href="/cetaksegel/{{ $value->spmID }} " class="btn btn-primary" target="_blank">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                        fill="currentColor" class="bi bi-file-lock2" viewBox="0 0 16 16">
                                        <path
                            ...
                            <td>
                                @if ($value->trans_type == 'multi')
                                    <a href="/cetaksjmp/{{ $value->header_id }} " class="btn btn-warning"
                                        target="_blank">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                            ...
                            <td>
                                <a wire:click="edit('{{ $value->spmID }}', '{{ $value->trans_type }}')"
                                    class="btn btn-warning btn-sm">PGI</a>
                            </td>
                            @php
