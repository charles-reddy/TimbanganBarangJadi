            ...
            <thead>
                <tr>
                    ...
                    <th class="col-md-1 sort @if ($sortColumn == 'driver') {{ $sortDirection }} @endif"
                        wire:click="sort('driver')">SO/SPPB</th>
                    <th class="col-md-1 sort @if ($sortColumn == 'driver') {{ $sortDirection }} @endif"
                        wire:click="sort('driver')">PO</th>
                    <th class="col-md-1 sort @if ($sortColumn == 'driver') {{ $sortDirection }} @endif"
                        wire:click="sort('driver')">Ekspedisi</th>
                    <th class="col-md-1 sort @if ($sortColumn == 'driver') {{ $sortDirection }} @endif"
                        wire:click="sort('driver')">No.Seal</th>
                    <th class="col-md-1 sort @if ($sortColumn == 'driver') {{ $sortDirection }} @endif"
                    ...
                    <th class="col-md-1 sort desc @if ($sortColumn == 'custName') {{ $sortDirection }} @endif"
                        wire:click="sort('custName')">Customer</th>
                    <th class="col-md-1 sort desc @if ($sortColumn == 'itemCode') {{ $sortDirection }} @endif"
                        wire:click="sort('itemCode')">Item Name </th>
                    <th class="col-md sort desc @if ($sortColumn == 'timbangin') {{ $sortDirection }} @endif"
                        wire:click="sort('timbangin')">Bobot IN </th>
                    <th class="col-md sort desc @if ($sortColumn == 'timbangout') {{ $sortDirection }} @endif"
                        wire:click="sort('timbangout')">Bobot OUT </th>
                    <th class="col-md sort desc @if ($sortColumn == 'netto') {{ $sortDirection }} @endif"
                        wire:click="sort('netto')">Netto </th>
                    <th class="col-md sort desc @if ($sortColumn == 'jam_in') {{ $sortDirection }} @endif"
                        wire:click="sort('jam_in')">Date IN </th>
                    <th class="col-md sort desc @if ($sortColumn == 'jam_out') {{ $sortDirection }} @endif"
                        wire:click="sort('jam_out')">Date Out </th>
                    <th class="col-md sort desc @if ($sortColumn == 'jam_out') {{ $sortDirection }} @endif"
                        wire:click="sort('jam_out')">Shift Tiket </th>
                    <th class="col-md sort desc @if ($sortColumn == 'jam_out') {{ $sortDirection }} @endif"
                        wire:click="sort('jam_out')">Shift Muat </th>
                </tr>
            </thead>
            <tbody>
                @foreach ($datascaleout as $key => $value)
                    <tr>
                        ...
                        <td>
                            @php
                                // Shift Tiket berdasarkan jamMuat dari create_t_m_s
                                if (is_null($value->shift_tiket)) {
                                    echo '-';
                                } else {
                                    echo $value->shift_tiket;
                                }
                            @endphp
                        </td>
                        <td>
                            @php
                                // Shift Muat berdasarkan jam_in (timbang masuk)
                                if (is_null($value->jam_in)) {
                                    echo '-';
                                } else {
                                    $jamIn = date('H:i', strtotime($value->jam_in));
